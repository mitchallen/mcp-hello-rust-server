...
IMAGE ?= mcp-hello-rust-server
BIN   ?= mcp-hello-rust-server
...
TAG ?= latest
PUBLISHED_IMAGE ?= $(REGISTRY)/mcp-hello-rust-server
CONTAINER ?= mcp-hello-rust
HTTP_PORT ?= 8000
...
help:
	...
	@echo "  make build        - Build the release binary (cargo build --release)"
	@echo "  make run          - Run the MCP server over stdio"
	@echo "  make run-http     - Run the MCP server over streamable HTTP (PORT, default 8000)"
	@echo "  make test         - Run the test suite (cargo test)"
	@echo "  make fmt          - Format the code (cargo fmt)"
	@echo "  make clippy       - Lint with clippy (warnings are errors)"
	@echo "  make check        - fmt --check + clippy + test (the CI gate)"
...
.PHONY: run-http
run-http:
	MCP_TRANSPORT=http HOST=0.0.0.0 PORT=$${PORT:-8000} cargo run --release
...
# The full local CI gate: formatting, lints, and tests.
.PHONY: check
check:
	cargo fmt --check
	cargo clippy --all-targets -- -D warnings
...
# Bump the version in Cargo.toml (+ Cargo.lock), commit, tag, and push.
...
release:
	...
	@cur=$$(grep -m1 '^version' Cargo.toml | sed -E 's/.*"([0-9]+\.[0-9]+\.[0-9]+)".*/\1/'); \
...
docker-smoke:
	...
	@curl -fsS -L --max-time 10 \
	  ...
	  | grep -q '"name":"mcp-hello-rust-server"' \
